...
if gh run watch "$RUN_ID" --exit-status; then
  osascript -e 'display notification "Deploy succeeded!" with title "Candito Tool" sound name "Glass"'
  echo "✅ Deploy complete!"
else
  osascript -e 'display notification "Deploy FAILED" with title "Candito Tool" sound name "Basso"'
